...
void MotionControllerNode::current_pose_callback(const geometry_msgs::msg::Pose::SharedPtr msg)
{
  current_pose_ = *msg;
...
geometry_msgs::msg::Twist MotionControllerNode::compute_control_command()
{
  ...
  if (distance_to_target > 0.01) {  // Only move if not at target
    ...
    // Extract current theta from quaternion orientation
    double current_theta = std::atan2(2.0 * (current_pose_.orientation.w * current_pose_.orientation.z +
